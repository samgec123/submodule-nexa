...
export default async function decorate(block) {
  const [carModelNameEl, logoLinkEl, variationTypeEL, , , , ...ctasEl] = block.children;
  const { ariaLabelLogo, ariaLabelHamburger } = await fetchPlaceholders();

  const variationType = variationTypeEL?.textContent?.trim() || '';
  const carModelName = carModelNameEl?.textContent?.trim() || '';
    ...
    navButtons.forEach((button) => {
      button.addEventListener('click', (event) => {
        if (variationType === NAV_VARIATIONS.variation1) {
          // if the button contains an href, let it redirect
          if (button.href.slice(-1) !== '#') {
            return;
          }

          // return if clicked on the same button again
          ...
          if (!newActiveTab) return; // if there is no tab with this index
  ...
  block.innerHTML = utility.sanitizeHtml(`
    <div class="secondary-navbar-container ${variationType}">
        <nav class="secondary-navbar">
            <a href="${logoLink}" aria-label="${ariaLabelLogo}" class="logo-container ${
  variationType === NAV_VARIATIONS.variation2 ? 'back-button' : ''
}">
              ${carModelName} 
            </a>
            ${
  ...
  if (variationType === NAV_VARIATIONS.variation2) {
    ...
    const section = document
      .querySelector('.secondary-navigation')
      .closest('.section');
    const navbar = document.querySelector('.secondary-navbar-container.variation1');
    const subnavbar = document.querySelector('.secondary-navbar-container.variation2');
    ...
    function resetNavbarStates() {
      if (navbar) {
        navbar.classList.remove('fade-out', 'fade-in');
    ...
    function handleScrollDown(currentScroll, sectionTop, activeSectionHeight) {
      const navLocation = document.querySelector('.location-wrapper');
      if (currentScroll >= sectionTop && currentScroll < sectionTop + activeSectionHeight) {
        ...
        if(navbar.style.position = 'fixed'){
          navLocation.classList.add('hideLocation');
        }
    ...
    window.addEventListener('scroll', () => {
      ...
      if (currentScroll < sectionTop) {
        ...
        if(navbar.style.position = 'relative'){
          navLocation.classList.remove('hideLocation');
        }
  ...
  block.querySelectorAll('.secondary-navbar-container.variation1 .nav-button').forEach((item) => {
    const data = {};
    data.componentTitle = `SubNav|${carModelName}`;
