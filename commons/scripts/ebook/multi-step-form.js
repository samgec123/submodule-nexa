...
import { render, h, createContext, cloneElement, Fragment } from '../vendor/preact.js';
import { useState, useRef, useEffect } from '../vendor/preact-hooks.js';
import { html } from '../vendor/htm-preact.js';
import { fetchPlaceholders, toClassName } from '../aem.js';
import { getLocalStorage, removeLocalStorage } from '../../utility/localStorage.js';
import { getEbookUserSession } from '../../utility/apiUtils.js';

export const getCleanSessionData = () => {
  let sessionDataString = sessionStorage.getItem('ebookGetSession');
  // console.log("sessionDataString..", sessionDataString)
  if (sessionDataString !== 'undefined') {
    sessionDataString = JSON.parse(sessionDataString);
    // console.log("sessionDataString..", sessionDataString)
    if (sessionDataString?.SESSION_INFO) {
      // eslint-disable-next-line no-useless-escape
      const unescapedString = sessionDataString?.SESSION_INFO?.replace(/\"/g, '"');
      const jsonData = JSON.parse(unescapedString) || null;
      // console.log("cleanData..", jsonData)
      const cleanData = {
        CHANNEL: sessionDataString?.CHANNEL,
        MOBILE: sessionDataString?.MOBILE,
        WEB_REF_ID: sessionDataString?.WEB_REF_ID,
        SESSION_INFO: jsonData,
      };
      return cleanData;
    }
  }
...
export default async function decorate(block, routes) {
  ...
  function MultiStepForm() {
    const mobileNumber = getLocalStorage('ebookNumber');
    const [firstRoute, secondRoute] = Object.keys(routes);
    const [formState, updateFormState] = useState({ mobileNumber });
    const [placeholders, setPlaceholders] = useState({});
    const [activeRoute, setActiveRoute] = useState(firstRoute);

    const getUserSessionApi = async () => {
      const result = await getEbookUserSession();
      if (!result.error) {
        sessionStorage.setItem('ebookGetSession', JSON.stringify(result.data));
        sessionStorage.setItem('ebookGetSessionParsed', JSON.stringify(getCleanSessionData()));
        const json = getCleanSessionData();
        return json.SESSION_INFO;
      }
      return null;
    };

    const navigateRoute = () => {
      const expireTime = getLocalStorage('ebookExpiresIn');
      if (new Date().getTime() < new Date(Number(expireTime)).getTime()) {
        return true;
      }
      return false;
    };
    ...
    useEffect(() => {
      ...
      if (step && routes[step]) {
        ...
        setActiveRoute(firstRoute);
        if (navigateRoute()) {
          handleSetActiveRoute(secondRoute);
        }
      }
    ...
    const context = { activeRoute, setActiveRoute, formState, updateFormState, handleSetActiveRoute, placeholders, getUserSessionApi };
    ...
    useEffect(() => {
      if (new Date().getTime() > Number(getLocalStorage('ebookExpiresIn'))) {
  ...
  block.querySelector('.login').classList.add('hide-content');
  document.querySelector('body').append(block.querySelector('.login'));
  block.textContent = '';
