...
const defaultBlockEditorSupport = {
  getState: () => null,
  setState: () => null,
  handleSelection: () => null,
};

async function importBlockEditorSupport(block) {
  try {
    const { blockName } = block.dataset;
    const m = await import(getBlockScriptPath(blockName, 'editor-support.js'));
    return { ...defaultBlockEditorSupport, ...m };
  } catch (e) {
    return defaultBlockEditorSupport;
  }
}
    ...
    if (block) {
      ...
      if (newBlock) {
        if (block.matches('.dynamic-block')) {
          // dynamic blocks manage their updates themselves, dispatch an event to it
        ...
        const blockEditorSupport = await importBlockEditorSupport(block);
        const state = blockEditorSupport.getState(block);
        ...
        block.remove();
        blockEditorSupport.setState(newBlock, state);
...
async function handleSelection(event) {
  ...
  if (resource) {
    ...
    if (block) {
      const blockEditorSupport = await importBlockEditorSupport(block);
      blockEditorSupport.handleSelection(element, event.detail?.selected);
    }
